...
export class CreateConferenceService{
  ...
  add(conference_name :string,chair_username: string,co_chair_username:string ,starting_date: Date,ending_date: Date,abstract_deadline: Date,paper_deadline: Date,bidding_deadline : Date,review_deadline:Date): void{
    ...
    c.starting_date = new MyDate(starting_date.getUTCDay(),starting_date.getMonth()+1,starting_date.getFullYear());
    c.ending_date = new MyDate(ending_date.getUTCDay(),ending_date.getMonth()+1,ending_date.getFullYear());
    c.abstract_deadline = new MyDate(abstract_deadline.getUTCDay(),abstract_deadline.getMonth()+1,abstract_deadline.getFullYear());
    c.paper_deadline = new MyDate(paper_deadline.getUTCDay(),paper_deadline.getMonth()+1,paper_deadline.getFullYear());
    c.bidding_deadline = new MyDate(bidding_deadline.getUTCDay(),bidding_deadline.getMonth()+1,bidding_deadline.getFullYear());
    c.review_deadline = new MyDate(review_deadline.getUTCDay(),review_deadline.getMonth()+1,review_deadline.getFullYear());
